...
def likelihood_func(w, X, y_train, likelihood_var):
    ...
    l_vec = np.exp((-(y - np.dot(x,w))**2)/(2*lv))/((2*pi*lv)**0.5)
    likelihood = np.prod(l_vec)
...
def get_posterior_params(X, y_train, prior, likelihood_var = 0.2**2):
    ...
    '''
    # assuming prior.mean is 0
    lv = likelihood_var
    XT = X.getT()
    XTX = XT*X
    sigI = prior['var'].getI()
    post_mean = (XTX+lv*sigI).getI() * (XT*y_train)
    post_var = ((XTX/lv)+sigI).getI()
...
def get_predictive_params(X_new, post_mean, post_var, likelihood_var = 0.2**2):
    ...
    pred_mean = post_mean.getT()*X_new
    pred_var = X_new.getT()*post_var*X_new + likelihood_var
